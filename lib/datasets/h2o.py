...
from lib.utils.processing_h2o_utils import process_text_h2o
from lib.utils.data_augment import (  # 数据增强
    augmentation, 
    augmentation_joints, 
    get_augm_rot, 
    get_augm_scale, 
...
class SequenceH2O(Dataset): # point encoder
    def __init__(
        self, 
        data_path,         # 数据集的 .npz 文件路径
        data_obj_pc_path,  # 物体点云模型的路径
        text_json,         # 包含动作文本描述的 JSON 文件路径
        max_nframes,       # 最大帧序列长度(用于限制单条数据的帧数量)
        data_ratio=1.0,    # 数据采样比例(用于控制使用数据集的多少，默认全量使用
        augm=False,        # 数据增强,默认关闭
        **kwargs
    ):
        super().__init__()
        ...
        print("H2O数据集读取已结束,用时: ", f"{time.time()-start_time:.2f}s")
        print(f"'H2O' 数据集总长度: {self.__len__()}")
...
class ContactH2O(Dataset): # point encoder
    def __init__(
        ...
        **kwargs
        ...
        with np.load(data_path, allow_pickle=True) as data:
            ...
            self.lcov_idx = data["lcov_idx"] # left contact object verts idx  接触物体点的索引
            self.rcov_idx = data["rcov_idx"] # right contact object verts idx
            self.is_lhand = data["is_lhand"]
            self.is_rhand = data["is_rhand"]
            self.action_name = data["action_name"]  # 动作名称
        with open(text_json, "r") as f:
            self.text_description = json.load(f)
    ...
    def __getitem__(self, index):
        ...
        if self.augm:  # 如果开启数据增强
            aug_scale = get_augm_scale(0.2).numpy()  # 生成数据增强中使用的随机缩放因子
